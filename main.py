...
def breedAI(stupid):
    ...
    print(len(stupid[(int(no_players))].coins))
    for i in range(int(no_players/2)-1,-1,-1):
        stupid.remove(stupid[i])
    ...
    print(stupid[(int(no_players / 2))].bestEver)
    return stupid
color=(0,0,0)
...
while not simOver:
    ...
    for i in range(no_players):
        if len(players[i].coins) >len(players[greatestind].coins):
            greatestind=i
            greatestnum=len(players[greatestind].coins)
    ...
    if players[0].currentAllele==200:
        players=breedAI(players)
        gen+=1
        if greatestnum>besterest:
            besterest=greatestnum
    textSurface = myfont.render("generation: "+ str(gen)+" best all: "+ str(round(besterest))+" best right now: "+str((round(len(players[greatestind].coins)))), True, (255,255,255))
